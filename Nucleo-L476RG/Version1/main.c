...
#define N_SAMPLES BUFFER_SIZE*2
...
int16_t processed_buff[N_SAMPLES] = {0};
uint8_t sender_buffer[N_SAMPLES * 2] = {0};
uint8_t start_datagram[] = "Start TX";

/* Buffer to store PCM data */
int32_t Buff[2048 * 2] = {0};
...
int main(void)
{
  ...
  // If any TX is ongoing, stop it and clear the buffer
  ...
  while(1){
  	  ...
  	  HAL_DFSDM_FilterRegularStart_DMA(&hdfsdm1_filter0, Buff, BUFFER_SIZE*2);


  	  if(!flag){

  		  // TX the start datagram firs
  		  HAL_UART_Transmit(&huart2, start_datagram, sizeof(start_datagram), HAL_MAX_DELAY);

  		  HAL_UART_Transmit(&huart2, sender_buffer, sizeof(sender_buffer), HAL_MAX_DELAY);

  		  // At this point, TX finished. Clear the TX buffer.
  		  HAL_UART_AbortTransmit(&huart2);
...
void HAL_DFSDM_FilterRegConvCpltCallback(DFSDM_Filter_HandleTypeDef *hdfsdm_filter)
{
	...
	// Once the buffer is full, stop DMA
	HAL_DFSDM_FilterRegularStop_DMA	(hdfsdm_filter);

	// Process the data
	// For each value in buff(2 samples) get 1 value in processed_buff(1 sample)
    if (flag)
    {

    	for (i = 0; i < BUFFER_SIZE; i++){
    		for(j = 0; j < 2; j++){
    			processed_buff[i*2 + j] = (j==0 ? ((Buff[i] & 0xFFFF0000) >> 16) : (Buff[i] & 0x0000FFFF)) >> 4;
    		}
    	}
    ...
    // TODO: Restart DMA and set circular mode
